...
def transform_data(service_account_path=SERVICE_ACCOUNT_PATH, **kwargs):
    ...
    # Retrieve the data from FK tables
    query = f"""
    SELECT 
        new_code,
        harvest_name 
    ...
    # FK Check for 'gh_actual' in 'gh_master'
...
# Load passed data to BigQuery with TRUNCATE logic
def load_passed_data(service_account_path=SERVICE_ACCOUNT_PATH, **kwargs):
    ...
    if not records_to_insert.empty:
        try:
            ...
            job_config = bigquery.LoadJobConfig(write_disposition="WRITE_TRUNCATE")  # Use WRITE_TRUNCATE to replace the data
            client.load_table_from_dataframe(insert_df, TARGET_TABLE_PASSED, job_config=job_config)
            logger.info(f"Inserted {len(insert_df)} records successfully, table was truncated first.")
        except Exception as e:
            logger.error(f"Failed to truncate and insert records: {str(e)}")
    else:
